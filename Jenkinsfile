    ...
    stages{
        ...
        stage("deploy"){
            steps{
                ...
                sshPublisher(publishers: [sshPublisherDesc(configName: 'kg666', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'sh /usr/local/fronted/report.sh', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: 'usr/local/fronted/', remoteDirectorySDF: false, removePrefix: 'coverage', sourceFiles: 'coverage/report.tar.gz', usePty: true)], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: true)])
                echo "deploy success"
            }
        }
    }
